...
interface OutcomeLLMTestTypeChartProps {
  data: HeatmapDataPoint[];
  title?: string;
}

export function OutcomeLLMTestTypeChart({
  data,
  title = "Valid test suite rates (O4)",
}: OutcomeLLMTestTypeChartProps) {
  // Get unique LLMs and create lines for each
  const uniqueLLMs = Array.from(new Set(data.map((d) => d.llm))).sort();

  // Group data by test type, with each LLM as a separate data point
  const testTypeGroups = new Map<string, Record<string, string | number>>();

  data.forEach((point) => {
    const testTypeKey =
      point.testType === "standard"
        ? "Standard"
        : point.testType === "boundary"
          ? "Boundary"
          : "Mixed";

    if (!testTypeGroups.has(testTypeKey)) {
      testTypeGroups.set(testTypeKey, { testType: testTypeKey });
    }

    const group = testTypeGroups.get(testTypeKey)!;
    group[point.llm] = point.O4_percentage;
  });

  const chartData = Array.from(testTypeGroups.values());

  // LLM-specific color mapping (matching the test case view colors)
