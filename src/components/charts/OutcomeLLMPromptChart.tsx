...
interface OutcomeLLMPromptDataPoint {
  llm: string;
  prompt_type: string;
...
interface OutcomeLLMPromptChartProps {
  data: OutcomeLLMPromptDataPoint[];
  title?: string;
}

export function OutcomeLLMPromptChart({
  data,
  title = 'Valid test suite rates (O4)',
}: OutcomeLLMPromptChartProps) {
  ...
  data.forEach((point) => {
    const promptLabel = point.prompt_type === 'zero_shot' ? 'Zero-Shot' :
                       point.prompt_type === 'few_shot' ? 'Few-Shot' : 'Chain-of-Thought';
      ...
      <ResponsiveContainer width="100%" height={450}>
        ...
        >
          ...
          <Legend
            ...
            iconSize={10}
          />
          {/* Lines with distinctive styling for each LLM */}
          {uniqueLLMs.map((llm, index) => {
            const color = getLLMColor(llm, index);
            return (
              <Line
